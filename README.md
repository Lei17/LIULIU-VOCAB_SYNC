...
- **词库导入** — 内置 9000 词 TOEFL 词库首次启动自动加载，同时支持 CSV / TSV / JSON 导入
...
前往 [Releases](https://github.com/Lei17/LIULIU-VOCAB_SYNC/releases) 页面下载最新版本。

1. 下载 `66背词看板-v1.1.0-win32-x64.zip`
2. 解压到任意目录
3. 运行 `66背词看板.exe` 即可使用，无需安装 Node.js
...
### v1.1.0 (2026-05-31)
- 新增内置 TOEFL 词库（9000 词），首次启动自动导入 SQLite，无需手动导入
- 启动时自动检测词库，若为空则从内置 JSON 批量种子写入
- WAL checkpoint 确保种子数据持久化到主数据库文件
